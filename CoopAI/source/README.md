...
15장. 갱신일 2026-09-19. 사용자 요청에 따라 기존 205 자료실 템플릿으로 제작했다. 공식 API 동작과 팀의 내부 설계를 구분한다.
...
- 독립 진행 슬라이드: 행동 AI의 판단 길이를 조절. 시간 단위는 설명용 가상 단계이며 실측이 아니다.
...
언리얼→Node의 게임 JSON, task_epoch, operation_id, 행동 목록, 취소·동기화·제안 조건은 우리 시스템의 설계 예시다. 행동 AI의 최종 모델과 실명별 역할은 미확정이다. AI 역할 수는 두 개로 확정했다. 학생 연락처·학번·API 키는 포함하지 않는다.
